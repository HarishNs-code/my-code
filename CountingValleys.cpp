...
int countingValleys(int n, string s) {
    int checkSeaLevel = 0, valleyCount = 0;
    for ( int i = 0 ; i < n; i++)
    {   
        if(s[i] == 'U')
        {
        ...
        {
            if((checkSeaLevel == 0) && (s[i] == 'U')) // checking the uphill because the count should not check or count the up fill flag.
            {
                valleyCount++;
            }
        }
    }
    std::cout << "valleyCount is :"<<valleyCount<< std::endl;
    return valleyCount;
}
